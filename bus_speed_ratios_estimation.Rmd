...
ratioTable2 <-ratioTable1.5 %>%
  group_by(FTGROUP,FTGNUM,areaNum)  %>%
...
ratioTable3 <- ratioTable2.1 %>%
  mutate(pknum = ifelse(PkOk == "dt", 1, ifelse(PkOk == "ok", 2, 3))) %>%
  mutate(ftat = as.numeric(paste0(pknum,FTGNUM,areaNum))) %>%
...
stat_smooth_data <- 
  ...
  mutate(FTGROUP = case_when(
    ...
    group == 4 ~ "FT-Minor Arterials",
    group == 5 ~ "FT-Principal Arterials",
    group == 2 ~ "FT-Expressways"
  )) %>%
