...
transit_line_file <- "data/TDM/Distrib_Network/_v832_SE19_Net19_2_OD_Station_Detail2.dbf"
...
### Step Six -- Manually Smooth Lines Again
...
### Step Seven -- Final Ratios
```{r ratios7, include = FALSE, warning= FALSE, message = FALSE}
ratioTable6 <- ratioTable5 %>%
  mutate(Ratio6 = case_when(
    # fix collectors/locals for all time periods
    ftat == 111 ~ .58,
    ftat == 112 ~ .6,
    ftat == 113 ~ .62,
    ...
    ftat == 324 ~ .6,
    ftat == 325 ~ .53,
    # fix major arterials for all time periods
    ftat == 131 ~ .85,
    ftat == 132 ~ .75,
    ftat == 133 ~ .65,
    ftat == 134 ~ .56,
...
```{r, echo = FALSE, warning=FALSE, message=FALSE}
ratios6graph <- ggplot(ratioTable6) +
  aes(x = AreaType, y = Ratio6, color = FTGROUP) +
  facet_wrap(~PkOk)+
  geom_point(size = 2)+ 
  scale_y_continuous( limits = c(0,1), expand = c(0,0) ) +
  theme_bw() + 
...
r_table4 <- ratioTable6 %>%
  pivot_wider(c(FTGROUP,PkOk),names_from = AreaType, values_from= Ratio6)
