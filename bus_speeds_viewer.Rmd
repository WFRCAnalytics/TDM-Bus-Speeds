...
uta_points <- tar_read(uta_points) %>%
  mutate(period2 = ifelse(grepl("m peak",period),"pk","ok")) %>%
  st_transform(4326) %>%
...
renderLeaflet({
  ...
  leaflet() %>%
    ...
    addCircleMarkers(lng = utaStops$lon, lat = utaStops$lat,
      popup = paste("Avgmphdwell",utaStops$Avgmphdwell),
      color = "red",
      radius = 6) %>%
    addPolylines(
...
renderPlot({
  ...
  if(input$Period == 1){
      return(ggplot(routeMap, aes(x = link_id))+
        geom_col(aes(y=EstAvgmphdwell, colour = "Observed"), alpha = .2) +
        geom_col(aes(y=P_SPEED1, colour = "Modeled"), alpha = .2) +
  ...
  if(input$Period == 2){
      return(ggplot(routeMap, aes(x = link_id))+
        geom_col(aes(y=EstAvgmphdwell, colour = "Observed"), alpha = .2) +
        geom_col(aes(y=O_SPEED1, colour = "Modeled"), alpha = .2) +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
        scale_fill_manual(values = c("blue","red")))
  }
})
